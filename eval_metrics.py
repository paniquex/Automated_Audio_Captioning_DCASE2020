...
def evaluate_metrics_from_files(pred_file: Union[Path, str],
    ...
    coco = COCO(str(ref_file), verbose=verbose)
    ...
    cocoEval.evaluate(verbose=verbose)
...
def evaluate_spider_from_files(pred_file: Union[Path, str],
                                ref_file: Union[Path, str],
                                verbose: bool = True) \
        -> Tuple[Dict[str, float], Dict[int, Dict[str, float]]]:
    """ Evaluate the translation metrics from annotation files with the coco lib
    ...
    cocoEval = COCOEvalCap(coco, cocoRes)
    cocoEval.params['audio_id'] = cocoRes.getAudioIds()
    cocoEval.evaluate_spider(verbose=verbose)
...
def evaluate_spider_from_lists(predictions: List[str],
                                ground_truths: List[List[str]],
                                ids: Union[List[int], None] = None,
                                verbose: bool = True) \
        -> Tuple[Dict[str, float], Dict[int, Dict[str, float]]]:
    ...
    metrics, per_file_metrics = evaluate_spider_from_files(pred_file, ref_file,
                                                            verbose=verbose)
...
def evaluate_metrics(prediction_file: Union[str, Path, List[Dict[str, str]]],
    ...
    :param verbose: if True, then output to console results
    type: bool
...
def evaluate_spider(prediction_file: Union[str, Path, List[Dict[str, str]]],
                     reference_file: Union[str, Path, List[Dict[str, str]]],
                     nb_reference_captions: int = 5,
                     verbose: bool = True) \
        -> Dict[str, Dict[str, Union[float, Dict[str, float]]]]:
    ...
    metrics, per_file_metrics = evaluate_spider_from_lists(predictions, ground_truths,
                                                            verbose=verbose)
